...
file = r'C:\Users\crrat\PycharmProjects\StreamCrawler\logs\housemovie_to_sources.p'
output = open(r'C:\Users\crrat\PycharmProjects\StreamCrawler\FilteredResults\housemovie_output', 'w+')
...
for frame in frames:
    try:
        ...
        host = re.sub(re.compile(r'\.[a-z]+'), '', host)\
            ...
            .replace('.to', '').replace('.net', '').replace('.me', '').replace('.eu', '').replace('embed.', '').replace('hd', '')

        host.lower()
    except:
        host = frame.lower().replace('.us', '')\
            .replace('.tv', '').replace('.com', '').replace('.co', '').replace('.to', '')\
            .replace('.net', '').replace('.me', '').replace('.eu', '').replace('.sx', '').replace('embed.', '').replace('hd', '')
